...
export function useGoogleMaps() {
  ...
  ): Promise<{ path: MapsLatLng[]; distanceKm: number; durationMinutes: number }> {
    const g = getGoogle()
    if (!g?.maps) {
      return Promise.reject(new Error('Maps not loaded'))
    }
    ...
    return new Promise((resolve, reject) => {
      service.route(
        ...
        (result, status) => {
          ...
          const rawPath = route.overview_path.map((p) => toLiteral(p))
          const path = samplePath(rawPath, 48)
          ...
          resolve({ path, distanceKm, durationMinutes })
        },
      )
    })
  }
